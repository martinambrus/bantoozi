...
- **T6:** the suggestion flow works end-to-end against the fake TypeSafe server; dismissed cards are
  excluded for 90 days; at most one admitted attempt per user per day through the durable
  `last_suggested_at` gate, while a run that sends nothing (too few items or a budget deferral) does
  not block a later trigger.
- **T7:**
