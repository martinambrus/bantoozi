...
plus the PR #2 review fixes `1e36c27`, `87d8e0e`, `82c51a4`, `551acf5` and `2bd9093` of 2026-09-26;
the per-task commits are in the table below). The full check passed after the last task commit, and
`pnpm dev` served `GET /api/v1/readyz → 200`. M1 and M2 start once this branch is merged to `main`
(§0.2). Deviations: D-1…D-10 in `docs/DECISIONS.md`; follow-up decisions in §17.3.
...
| M0-T5 | `packages/db`: schema, grants, RLS, SQL functions, pg-boss schema and queues in the migrate job, `withTenant`, factories (testing part 2), schema parity and RLS tests | T2, T3, T4 | A | 02 all; 03 §2 | ✓ `a3b7ba6`, `84f6b02`, `1e36c27`, `82c51a4`, `551acf5`, `2bd9093` |
...
- **T6:** `pnpm db:seed` runs idempotently. It inserts only `card_text_mode`,
  `question_sets.active = {}` and `language_modes` from `LANGUAGE_MODES` (spec 02 §2), and provides
  the hooks M2 fills (topics, question sets, library). Running it twice changes nothing, even with a
  different `LANGUAGE_MODES` the second time (a test).
- **T7:**
...
| M1-T7 | Migration for `articles.has_video`, `body_image_count` and `media_changed_at`; worker handlers: `feed.schedule`, `feed.fetch` (ingest §7, redirect merge §9), `article.extract` (alias/merge), `feeds.lang_hint` upkeep, `resetArticleAnswers`, inference eligibility and durable bookmark capture | T1–T5 | D | 03 §1–3, §7–9; 05 §5.6 |
...
- **T7:**
  - Handlers implement spec 03 §7: exact URL and feed-scoped GUID identity; fuzzy candidates are
    ...
    change increments the input revision and calls `resetArticleAnswers`; on a stale article its
    stale-preserving form keeps `stale` and schedules no pipeline or match work.
  - They implement the §9 feed updates, including the permanent-redirect merge (subscriptions,
    `feed_items`, scopes and rule values moved; a full rank enqueued). An active source-only
    subscription restarts its activation boundary at the merge, so the target's earlier items
    create no automatic demand for it.
  ...
  - A repeat fetch that adds only a video enclosure (same `content_hash`, no new `feed_items` row)
    sets `media_changed_at` and records an incremental rank for the subscribers of every carrier; a
    fetch that stores the same values records none.
  ...
  - An article merge advances the surviving `user_article` row's `state_version` past both inputs,
    so an offline action against either pre-merge version returns `STALE_STATE` (tested).
...
- **T9:**
  ...
  - The cluster candidate SQL of spec 05 §6, with the per-feed rule in code; a similar article from
    an off or unselected training feed is never a candidate, while an authorized one whose facets
    predate a model-pin change still is. Merging two clusters
    remaps `mute_story` rules to the survivor and records full-rank intents for the affected users.
    A new membership records `cluster_set_id`, and switching the cluster set leaves existing
...
- **T3:**
  ...
  - Token-cookie auth, one-time URL exchange/redaction and rater-scoped access. An expired or
    revoked token cannot be exchanged, revocation ends existing cookie sessions at once, and a
    reissued token works again with the rater's ratings intact while the old token's sessions end.
...
- **T2:**
  - Every row of the `request-code` decision table is tested, including the admin bootstrap with an
    empty `invites` table and a soft-deleted user, and with a stored `signup_mode` that differs from
    `SIGNUP_MODE` (the stored mode decides both the request and the verification).
  ...
  - The waitlist upsert; signup deletes the matching waitlist row.
...
- **T4:**
  ...
  - Unsubscribing keeps that feed's completed analysis requests, whose frozen results still serve
    learning; pending/running ones lose their demand and the worker cancels them.
- **T5:**
  ...
  - Library localization (sk); suggestions dismiss. Adopting a suggested library card removes it
    from `GET /cards/suggestions`, and so does switching the active `suggest` set or the Jev model
    pin; suggestion calls never use the LLM fallback.
- **T6:**
...
- **T7:**
  ...
  - Saved snapshot access survives upstream 404/deletion, unsubscribing and ordinary body purges;
    ...
    when `snapshotId` and its captured `contentRevision` match; a mismatched pair is `STALE_STATE`.
- **T8:** value validation per kind; creating or deleting a rule enqueues `user.rank {full:true}`.
  A `mute_story` rule without `expiresInDays` is rejected (and the schema rejects a null expiry).
- **T9:**
  ...
  - With no DB row, a fresh worker heartbeat listing an env key yields `source: 'env'`; the API
    process has no env key.
  - A `translate` language mode or `english` card text mode is refused while LibreTranslate is
    down, and accepted once its health probe lists the needed languages.
...
- **T2:**
  ...
  - A feed view that keeps one of two authorized carriers shows the cached global score, including
    a card scoped to the other carrier.
  - Explicit precedence tests:
    - a hide rule hides a stale item (the hide check runs first)
    - a stale item stays `new` unless a current explicit selection covers it, which ranks it from
      its completed answers
    - when P is null (step 4d), no floor or cap applies
    - seen_story beats a floor
    - seen_story keeps a degraded item in Everything (the degraded Maybe lane does not override it)
...
- **T4:**
  - The dirty-set SQL covers every freshness trigger in spec 06 §7, including time, undo, model,
    translated card text, media signal and subscription changes (a test each), and windows by
    subscribed carrier arrival rather than global first-seen time, while a completed explicit
    selection of an older article is ranked outside that window, from the answers its request
    published to the current caches; a model-pin rebuild refills those answers for it too.
...
- **T6:** the suggestion flow works end-to-end against the fake TypeSafe server; dismissed cards are
  ...
  undismissed suggestions, and a run whose evidence has expired clears them; at most one admitted
  attempt per user per day through the durable `last_suggested_at` gate, while a run that sends
  nothing (too few items or a budget deferral) does not block a later trigger; a worker whose lease
  expired or was reclaimed before its spend reservation gets no reservation, sends nothing and does
  not stamp `last_suggested_at`.
- **T7:**
...
| M8-T2 | Housekeeping jobs: `house.purge-auth`, `house.reconcile`, `house.archive`, `house.purge-articles`, `house.purge-bodies`, cold bookmark compression, `house.purge-engine-calls`, `house.retire-cards`, `house.purge-users`, `house.reenrich`, `house.rematch`, `house.translate-cards`, `house.retranslate-skipped` | — | B | 11 §5–6, §8; 05 §2; 07 §5 |
...
- **T2:** each listed job exists and is scheduled (or triggered on demand), and has an idempotency test
  ...
  feed's carrier arrival, so a newly carried older article is not archived on arrival (tested). An
  automatic archive increments `state_version`, so an offline `unread` made before it returns
  `STALE_STATE` (tested).
  A card that `house.translate-cards` skipped for lack of demand is translated and rematched after
  its holder activates a feed (tested).
  `house.reenrich` re-enqueues an admitted article whose carrier arrival was 10 days ago, and a
  `language_modes` change re-enters that language's window articles, translating first when the
  language is now `translate`; an already enriched native article is then reset through
  `resetArticleAnswers` and re-enriched from its translation, and switching back to `native`
  resets a translated article's answers and cluster membership before re-enrichment (tested). A
  pending plain `article.translate` job for the same article absorbs neither the mode-change job
  nor a `forceTier2` job (tested).
  ...
  (tested). Unused invites 30 days past expiry, used invites whose two accounts are both erased,
  and waitlist rows older than 365 days are purged (tested). A feed 30 days past its last subscription with no remaining references is purged,
  while one carrying a bookmarked article or re-subscribed within the grace period stays, and one
  with an explicit `allow`/`block` image preference stays, so re-subscribing later restores that
  choice (tested).
  An expired `reserved` spend reservation becomes `uncertain`, and an `uncertain` one is
  settled at its reserved amount 7 days after its day, then purged with its call audit (tested).
  A tier-2 translation skipped for `no_key`, `cap` or `budget` is not retried by redelivery or a
  budget reset, but `house.retranslate-skipped` replaces it for a still-demanded window article and
  a changed translation re-enriches that article (tested).
  30 days after retirement, an unheld card's answers and then the card itself are deleted unless a
  library version, publication request or eval row references it; a card reused during that grace
  is un-retired and kept (tested).
...
| 2026-09-26 | Media signals R2 (§17.4): the personal model gains `has_video` and a bucketed in-body image density per 500 words, both detected at ingestion and extraction before sanitizing removes the media. Specs 02, 03 and 06 updated; plan: M1-T3, T5, T7, M5-T4 and M7-T1 |
