...
- **T6:** the suggestion flow works end-to-end against the fake TypeSafe server; dismissed cards are
  ...
  ago can be suggested again (the upsert clears the old dismissal); at most one admitted attempt
  per user per day through the durable `last_suggested_at` gate, while a run that sends nothing
  (too few items or a budget deferral) does not block a later trigger.
- **T7:**
