...
- **T5:**
  ...
  - Library localization (sk); suggestions dismiss. Adopting a suggested library card removes it
    from `GET /cards/suggestions`, and so does switching the active `suggest` set or the model pin,
    including a change of only the Ollama fallback models while the fallback is enabled.
- **T6:**
...
- **T2:** each listed job exists and is scheduled (or triggered on demand), and has an idempotency test
  ...
  `resetArticleAnswers` and re-enriched from its translation (tested).
