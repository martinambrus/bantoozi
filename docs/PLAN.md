...
- **T5:**
  ...
  - Library localization (sk); suggestions dismiss. Adopting a suggested library card removes it
    from `GET /cards/suggestions`, and so does switching the active `suggest` set.
- **T6:**
...
- **T2:** each listed job exists and is scheduled (or triggered on demand), and has an idempotency test
  ...
  (tested). Unused invites 30 days past expiry and waitlist rows older than 365 days are purged
  (tested). A feed 30 days past its last subscription with no remaining references is purged,
