...
| `feeds` | while subscribed. 30 days after the last subscription ends (`unsubscribed_at`), a feed with no `feed_items`, `analysis_requests`, `eval.rater_feeds` row or merged feed pointing at it is deleted with its `user_feed_preferences` and `feed_cards`. A bookmarked article keeps its `feed_items`, and so keeps its feed |
