...
4. Stop consumption and outbox claiming in the old workers, drain bounded in-flight work, then replace
   ...
   is outstanding (§2), without that profile, so no work waits on a queue nothing consumes.
...
| Service health | missing worker/housekeeping heartbeat (including `worker-laya` while `engine.laya` lists a language or Laya work is outstanding, §2), repeated OOM/restarts, exhausted DB pool, or external readiness failure; external monitor covers full-host failure |
