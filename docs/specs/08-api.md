...
| `DELETE /subscriptions/:feedId` | — | Delete, and in the same transaction cancel this user's pending/running `analysis_requests` for the feed (completed ones stay for learning, spec 11 §5); then `refresh_feed_subscribers`, `refresh_feed_cards`, `user.rank {full}` |
