...
| `GET /cards/suggestions` | — | `[{card, score}]`: not dismissed, produced by the active `suggest` set, and never a card the user currently holds as an interest card or label (for example after adopting it from the library or creating it again) |
