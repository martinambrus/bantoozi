...
| `article.translate` | `{articleId, forceTier2?: boolean, replaceSkipped?: boolean, modeChange?: boolean}` | extract, `user.rank`, `house.retranslate-skipped`, `house.reenrich` after a language-mode change (spec 07 §3) | 4 | 1 | `stately`, key `translate:<id>` (`retranslate:<id>` with `replaceSkipped`) |
