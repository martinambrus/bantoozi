...
| `article.translate` | `{articleId, forceTier2?: boolean, replaceSkipped?: boolean}` | extract, `user.rank`, `house.retranslate-skipped` (spec 07 §3) | 4 | 1 | `stately`, key `translate:<id>` (`retranslate:<id>` with `replaceSkipped`) |
...
| `house.reenrich`, `house.translate-cards` | `{since?: iso, lang?: string}` (`lang` only for a language-mode, `engine.laya` or Laya checkpoint change) | admin action or model-pin change (spec 05 §2, spec 07 §5) | 1 | 1 | `singleton` |
